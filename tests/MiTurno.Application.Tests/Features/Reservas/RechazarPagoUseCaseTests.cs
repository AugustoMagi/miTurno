...
public class RechazarPagoUseCaseTests
{
    ...
    private readonly RechazarPagoUseCase _useCase;

    public RechazarPagoUseCaseTests()
    {
        _useCase = new RechazarPagoUseCase(
            _negocioRepository, _recursoRepository, _reservaRepository, _clienteRepository,
            _emailNotificador, _unitOfWork);
    }

    private (Negocio negocio, Reserva reserva, Cliente cliente) EscenarioValido()
    {
        ...
        return (negocio, reserva, cliente);
    }

    [Fact]
    public async Task ExecuteAsync_ConPagoPendiente_LoRechazaCancelaLaReservaYNotifica()
    {
        ...
        await _emailNotificador.Received(1).NotificarReservaRechazadaAsync(
    ...
    [Fact]
    public async Task ExecuteAsync_ConPagoYaRechazado_DevuelveFailureDelDominioSinNotificar()
    {
